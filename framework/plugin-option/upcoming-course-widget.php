<?php
/**
 * Plugin Name: Goodlayers Upcoming Course Widget
 * Plugin URI: http://goodlayers.com/
 * Description: A widget that show Upcoming course ( Specified by category ).
 * Version: 1.0
 * Author: Goodlayers
...
add_action('widgets_init', 'gdlr_lms_upcoming_course_widget');
if( !function_exists('gdlr_lms_upcoming_course_widget') ){
	function gdlr_lms_upcoming_course_widget() {
		register_widget( 'Goodlayers_Lms_Upcoming_Course' );
	}
}

if( !class_exists('Goodlayers_Lms_Upcoming_Course') ){
	class Goodlayers_Lms_Upcoming_Course extends WP_Widget{

		// Initialize the widget
		function __construct() {
			parent::__construct(
				'gdlr-lms-upcoming-course-widget', 
				__('Goodlayers Upcoming Course Widget','gdlr-lms'), 
		...
		function widget( $args, $instance ) {
			...
			$query_args['meta_key'] = 'gdlr_course_start_date';
			$query_args['meta_type'] = 'DATE';
			$query_args['meta_query'] = array(
				'key'     => 'gdlr_course_start_date',
				'compare' => '>=',
				'value'   => date('Y-m-d'),
				'type'    => 'DATE'
			);
			...
			if($query->have_posts()){
				...
				while($query->have_posts()){ $query->the_post();		
					...
					echo '<span class="gdlr-head">' . __('Start Date', 'gdlr-lms') . '</span> ';
