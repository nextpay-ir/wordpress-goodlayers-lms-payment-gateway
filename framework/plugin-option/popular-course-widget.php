<?php
/**
 * Plugin Name: Goodlayers Popular Course Widget
 * Plugin URI: http://goodlayers.com/
 * Description: A widget that show popular course ( Specified by category ).
 * Version: 1.0
 * Author: Goodlayers
...
add_action('widgets_init', 'gdlr_lms_popular_course_widget');
if( !function_exists('gdlr_lms_popular_course_widget') ){
	function gdlr_lms_popular_course_widget() {
		register_widget( 'Goodlayers_Lms_Popular_Course' );
	}
}

if( !class_exists('Goodlayers_Lms_Popular_Course') ){
	class Goodlayers_Lms_Popular_Course extends WP_Widget{

		// Initialize the widget
		function __construct() {
			parent::__construct(
				'gdlr-lms-popular-course-widget', 
				__('Goodlayers Popular Course Widget','gdlr-lms'), 
				array('description' => __('A widget that show popular courses', 'gdlr-lms')));  
		}
